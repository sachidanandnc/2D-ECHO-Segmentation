...
def test_binary_positive_weight_is_supported():
    logits=torch.zeros(1,1,16,16,requires_grad=True)
    target=torch.zeros_like(logits); target[:,:,4:12,4:12]=1
    loss,_=SegmentationLoss("binary",bce_pos_weight=1.25)(logits,target)
    assert torch.isfinite(loss)
    loss.backward()
