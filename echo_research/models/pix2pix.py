...
class Pix2PixResearchModel(nn.Module):
    def __init__(
        ...
        self.generator = UNetGenerator(1, out_channels, base_channels, norm_type=norm_type)
        self.discriminator = PatchDiscriminator(1, mask_channels, base_channels, norm_type=norm_type)
        if weight_init != "default":
            self.apply(lambda module: self._initialize_weights(module, weight_init))

    @staticmethod
    def _initialize_weights(module: nn.Module, weight_init: str) -> None:
        if weight_init != "normal":
            raise ValueError(f"Unknown weight initialization {weight_init!r}; choose default or normal")
