...
def build_test_loader(cfg):
    ...
    return _build_frame_eval_loader(cfg, te, "test")


def build_validation_loader(cfg):
    _, va, _ = split_ids_from_config(cfg)
    return _build_frame_eval_loader(cfg, va, "val")


def _build_frame_eval_loader(cfg, ids, split):
    ds = _dataset(cfg, ids, split, paired=False)
    tcfg = cfg["training"]
    workers = int(tcfg.get("num_workers", 4))
...
def build_model(cfg):
    return Pix2PixResearchModel(
        ...
        norm_type=cfg["model"].get("normalization", "batch"),
        weight_init=cfg["model"].get("weight_init", "default"),
    )
