...
from echo_research.inference import binary_inference_options, binary_masks_from_probabilities
...
def main():
    ...
    ap.add_argument("--inference-policy",default=None)
    ...
    _,_,test_ids=split_ids_from_config(cfg); verify_run_split(args.checkpoint,"test",test_ids,required=True); d=cfg["data"]; data_dir=resolve_data_dir(d["root"],d.get("nifti_dir","database_nifti"))
    ...
    for pid in test_ids:
        ...
        for view in ("2CH","4CH"):
            for phase in ("ED","ES"):
                ...
                x,_=tfm(img,gt,task="binary"); logits=model.generator(x[None].to(device)); probability=torch.sigmoid(logits)
                # Restore probabilities to the source grid before thresholding/postprocessing.
                probability_back=torch.nn.functional.interpolate(probability,size=gt.shape,mode="bilinear",align_corners=False)[0,0].cpu().numpy()
                pred_back=binary_masks_from_probabilities(probability_back,threshold,keep_largest,fill_holes)[0]
